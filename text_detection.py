...
for x, b in enumerate(boxes.splitlines()):
    if x != 0:
        ...
        if len(b) == 12 and len(b[11]) == 10 and (
                re.findall("\w\w\w\w\w\d\d\d\d\w", b[11]) or re.findall("\d\d/\d\d/\d\d\d\d", b[11])):
            x, y, w, h = int(b[6]), int(b[7]), int(b[8]), int(b[9])
            cv2.putText(img, b[11], (x, y + 50), cv2.FONT_HERSHEY_COMPLEX, 1, (100, 255, 255), 2)
